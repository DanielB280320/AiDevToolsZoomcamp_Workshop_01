...
**Status: design only — no application code exists yet.** There is no
`manage.py`, no settings package and no apps. The first task is to create them
(see `_docs/tasks.md` task 1).
...
- `python manage.py runserver` - dev server (once task 1 lands)
